...
describe('PaginatedTable', () => {
  ...
  test('pagination', async () => {
    ...
    expect(wrapper.text()).include('Show 5 | 10 | 20 | 40 | 60 results per page.')
  ...
  test('page selection', async () => {
    const wrapper = mount(PaginatedTable, 
      ...
      await wrapper.find('.btn-group.btn-group:nth-child(2) button').trigger('click')
      const bottomPaginationThree = wrapper.find('.container.paging')
      expect(bottomPaginationThree.text()).toBe('<< first< previous12')
      expect(bottomPaginationThree.findAll('.btn-group div')[1].find('button').element.classList).toContain('active')
